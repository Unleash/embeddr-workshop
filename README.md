...
Prerequisites: Node.js 22 or later, and an Unleash instance with a
server-side Client token.

1. Copy `server/.env.example` to `server/.env` and fill in the Unleash
   connection values. The frontend needs no configuration for local use;
   `.env.example` documents `VITE_BACKEND_URL` for deployed setups.
...
| Path | What it is |
|------|------------|
| `src/` | The frontend. No Unleash SDK, no flag logic, no token. |
| `server/` | The backend. Flag evaluation, openers, the ick impact metric. See `server/README.md`. |
| `WORKBOOK.md` | The attendee workbook. Start here at the workshop. |
| `FALLBACKS.md` | Plain commands for every assistant-driven step. |
